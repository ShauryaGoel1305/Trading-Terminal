...
// `enabled`/`model` are lifted too (from App.tsx's aiStatus poll) so this
// view doesn't run its own redundant /ai/status fetch.
interface Props {
  messages: ChatMsg[];
  busy: boolean;
...
export function AiAnalysisView({ messages, busy, onSend, onQuickBrief, onStop, onClear, activeSymbol, enabled, model, loading }: Props) {
  if (loading) {
