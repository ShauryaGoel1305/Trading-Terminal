...
const GENERIC_SUGGESTIONS = (symbol?: string) => [
  { label: symbol ? `📊 Full brief on ${symbol}` : "📊 Full brief on a ticker", prompt: `Give me a full investment brief on ${symbol ?? "NVDA"}.`, quickBrief: true },
...
export function AiChatPanel({ messages, busy, onSend, onQuickBrief, onStop, activeSymbol, compact }: Props) {
          ...
          <div className={`h-full flex flex-col items-center justify-center text-center gap-4 ${compact ? "py-3" : "py-8 gap-5"}`}>
            ...
            <div className={`grid grid-cols-1 ${compact ? "" : "sm:grid-cols-2"} gap-2 max-w-lg w-full`}>
              {GENERIC_SUGGESTIONS(activeSymbol).map((s) => (
                <button
                  key={s.label}
                  onClick={() => (s.quickBrief && activeSymbol && onQuickBrief ? onQuickBrief(activeSymbol) : onSend(s.prompt))}
      ...
      >
        ...
        {busy && onStop ? (
          <button
            type="button"
            onClick={onStop}
            className="px-3 py-2 text-2xs uppercase tracking-wide rounded-lg border border-term-red text-term-red hover:bg-term-red hover:text-black transition-all duration-200"
          >
            Stop
          </button>
        ) : (
